...
1. (R11, A1), (R12, R21), (R22, C1) 총 3개의 네트워크를 구성하되, 서로의 주소 범위가 겹치지 않게 설정하는데, R11 주소와 R12 주소가 이미 105.198.14.0/24 까지를 공유하고 있다. 따라서 3개의 네트워크가 구분되기 위해서는 최소한 서브넷 마스크 /26을 사용해야 다음과 같이 구분할 수 있다.
	- 105.198.14.0 ~ 105.198.14.63 (1구역)
	- 105.198.14.64 ~ 105.198.14.127 (2구역)
	- 105.198.14.128 ~ 105.198.14.191 (3구역)
	- 105.198.14.192 ~ 105.198.14.255 (4구역)
2. 서브넷 마스크 /26을 사용하고, A1은 R11 주소가 들어있는 1구역 범위의 주소를, R21은 4구역 범위의 주소를, R22와 C1은 2구역이나 3구역 중 아무 구역이나 선택해 한 구역 안에 들어있게 끔 설정한다.
3. 라우팅 테이블 
4. default -> 같은 네트워크 상의 보내고자하는 쪽 인터페이스 IP 주소를 입력
## Level 8
